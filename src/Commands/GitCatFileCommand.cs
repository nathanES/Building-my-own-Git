...
public class GitCatFileCommand
{
    ...
    private Result<string> ValidateAndExtractBlobContent(Memory<byte> uncompressedBlob)
    {
        ...
        var (blobType, blobLength, blobContent ) = decomposeResult.Response;
    ...
    private Result<(string BlobType, string BlobLength, string BlobContent)> DecomposeBlob(
        Memory<byte> uncompressedBlob)
    {
        ...
        return Result<(string, string, string)>.Create((blobType, blobLength, blobContent));
    }
...
public static class GitCatFileErrors
{
    ...
    public static readonly Error BlobNotFound =
        new Error("BlobNotFound", "The object with the specified SHA-1 hash could not be found.");

    public static readonly Error BlobTypeInvalid =
        new Error("BlobTypeInvalid", "The object is not a valid 'blob' type.");

    public static readonly Error BlobLengthInvalid = new Error("BlobLengthInvalid",
        "The blob's content length does not match the expected length.");
    ...
    public static Error DecompressionFailed(string message) =>
        new Error("DecompressionFailed", $"Failed to decompress the blob: {message}");
}